...
import { getFunction, getReceiveInterface, getConstructorInterface, visitContracts, tMakeFullTypeDefinition } from './txHelper'
...
export class TxListener {
  ...
  constructor (opt, executionContext) {
    ...
    opt.event.udapp.register('transactionExecuted', (error, from, to, data, lookupOnly, txResult) => {
      ...
      if (!this.executionContext.isVM()) {
        const { tx } = txResult
        addExecutionCosts(txResult, tx, null)
  ...
  _resolveTx (tx, receipt, cb) {
    ...
    if (!tx.to || tx.to === '0x0') { // testrpc returns 0x0 in that case
      ...
      const to = utils.addressToHex(tx.to)
      contract = this._resolvedContracts[to]
      if (!contract) {
        if (!this.executionContext.isVM()) {
          return this.executionContext
            ...
            .then(result => {
              if (result.runtimecode) {
                const contract = this._tryResolveContract(
                  result.runtimecode,
                  contracts,
                  false
                )
                if (contract) {
                  this._resolvedContracts[to] = contract
  ...
  _tryResolveContract (codeToResolve, compiledContracts, isCreation) {
    ...
    visitContracts(compiledContracts, (contract) => {
      ...
      if (utils.compareByteCode(codeToResolve, '0x' + bytes)) {
        found = contract
        return true
      }
    })
  ...
  _decodeInputParams (data, abi) {
    ...
    utils.tConvertTypes(inputTypes)

    const abiCoder = new ethers.utils.AbiCoder()
    const decoded = abiCoder.decode(inputTypes, data)
    const ret = {}
    for (var k in abi.inputs) {
      ret[abi.inputs[k].type + ' ' + abi.inputs[k].name] = abi.inputs[k].type == 'address' ? utils?.addressToBase58(decoded[k]) : decoded[k]
    }
    return ret
  }
}
