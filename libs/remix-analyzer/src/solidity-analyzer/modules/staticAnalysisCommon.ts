...
const basicFunctionTypes: Record<string, string> = {
  TOKENBALANCE: buildFunctionSignature([basicTypes.UINT], [], false),
  TRANSFERTOKEN: buildFunctionSignature([basicTypes.UINT, basicTypes.UINT], [], false),
...
const lowLevelCallTypes: Record<string, Record<string, string>> = {
  TOKENBALANCE: { ident: 'tokenBalance', type: basicFunctionTypes.TOKENBALANCE },
  TRANSFERTOKEN: { ident: 'transferToken', type: basicFunctionTypes.TRANSFERTOKEN },
...
function isInteraction (node: FunctionCallAstNode): boolean {
  return isLLCall(node.expression) || isLLSend(node.expression) || isExternalDirectCall(node) || isTransfer(node.expression) || isTokenTransfer(node.expression) ||
...
function isTokenTransfer (node: MemberAccessAstNode): boolean {
  return isMemberAccess(node,
    exactMatch(util.escapeRegExp(lowLevelCallTypes.TRANSFERTOKEN.type)),
    undefined, matches(basicTypes.ADDRESS, basicTypes.PAYABLE_ADDRESS), exactMatch(lowLevelCallTypes.TRANSFERTOKEN.ident))
}
