...
import { isLoop, isTransfer, isTokenTransfer, getCompilerVersion } from './staticAnalysisCommon'
import {
      ...
      if (node.body && node.body.nodeType === 'Block') {
        transferNodes = node.body.statements.filter(child =>
          (child.nodeType === 'ExpressionStatement' &&
                          ...
                          isTokenTransfer(child.expression.expression) &&
                          isTransfer(child.expression.expression)))
      } else if (node.body && node.body.nodeType === 'ExpressionStatement' && node.body.expression.nodeType === 'FunctionCall' && isTransfer(node.body.expression.expression) && isTokenTransfer(node.body.expression.expression)) { transferNodes.push(node.body) }
