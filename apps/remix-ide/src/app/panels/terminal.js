...
function createPreWrappedTextNode (text) {
...
function sanitizeTerminalLinks (el) {
  el.querySelectorAll('a[href]').forEach((anchor) => {
    ...
    try {
      ...
      if (!TERMINAL_ALLOWED_LINK_PROTOCOLS.includes(parsed.protocol)) {
...
class Terminal extends Plugin {
  constructor (opts, api) {
    ...
    self._JOURNAL = []
    self._jobs = []
    self._pendingTxCountInterval = null
    self._INDEX = {}
    self._INDEX.all = []
    self._INDEX.allMain = []
  ...
  onActivation () {
    this.startPendingTxCountPolling()
  ...
  onDeactivation () {
    this.stopPendingTxCountPolling()
  ...
  startPendingTxCountPolling () {
    if (this._pendingTxCountInterval || !this._view.pendingTxCount) return
    this._pendingTxCountInterval = setInterval(async () => {
      try {
        this._view.pendingTxCount.textContent = String(this.blockchain.pendingTransactionsCount())
      } catch (err) {
        console.error('Failed to fetch pending tx count:', err)
      }
    }, 1000)
  }

  stopPendingTxCountPolling () {
    if (!this._pendingTxCountInterval) return
    clearInterval(this._pendingTxCountInterval)
    this._pendingTxCountInterval = null
  }
  ...
  log (message) {
    ...
    if (typeof command === 'function') {
      if (typeof message.value === 'string' && message.type === 'html') {
        ...
        el.innerHTML = DOMPurify.sanitize(message.value, {
          ALLOWED_TAGS: ['a', 'b', 'p', 'em', 'strong', 'div', 'span', 'ul', 'li', 'ol', 'hr'],
          ALLOWED_ATTR: ['href', 'target', 'rel']
        })
        sanitizeTerminalLinks(el)
        el.querySelectorAll('a[href]').forEach((anchor) => {
  ...
  render () {
    ...
    `
    self.startPendingTxCountPolling()
    ...
    var intro = yo`
      <div><div> - Welcome to Tron IDE ${packageJson.version} - </div><br>
      <!--
      ...
      <ul class=${css2.ul}>
        <li><a target="_blank" href="https://github.com/tronprotocol/tronweb">TronWeb 4.0.0</a></li>
      ...
      return function logger (args, scopedCommands, append) {
        ...
        var values = javascriptserialize.apply(null, args.filter(filterUndefined)).map(function (val, idx) {
          if (typeof args[idx] === 'string') {
            val = createPreWrappedTextNode(args[idx])
          }
