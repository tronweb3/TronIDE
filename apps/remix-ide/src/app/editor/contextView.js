...
class ContextView {
  ...
  _render (node, nodeAtCursorPosition) {
    ...
    const showGasEstimation = () => {
      // Gas estimation is intentionally disabled on Tron IDE: the TVM uses an
      // energy/bandwidth resource model rather than EVM gas, so EVM-style gas
      // estimates would be misleading. Kept as a no-op until a TVM-native energy
      // estimation is implemented (see contextualListener.gasEstimation).
      return ''
    }
