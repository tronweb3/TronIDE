...
const tronAddressLinks = {
  main: 'https://tronscan.org/#/address/',
  shasta: 'https://shasta.tronscan.org/#/address/',
  nile: 'https://nile.tronscan.org/#/address/'
}

function tronAddressLink (networkId, address) {
  return tronAddressLinks[networkId] ? tronAddressLinks[networkId] + address : null
}

function deployedContractAddressLink (blockchain, networkId, address) {
  if (!address || !blockchain || blockchain.getProvider() === 'vm') return null
  return tronAddressLink(networkId, address)
}
...
UniversalDAppUI.prototype.renderInstanceFromABI = function (contractABI, address, contractName, contract) {
  ...
  const networkStatus = this.blockchain.getCurrentNetworkStatus && this.blockchain.getCurrentNetworkStatus()
  const networkId = networkStatus && networkStatus.network && networkStatus.network.id

  const base58Address = remixLib.util.addressToBase58(address)
  const addressLink = deployedContractAddressLink(this.blockchain, networkId, base58Address)
      ...
      <div class="input-group ${css.nameNbuts}">
        <div class="${css.titleText} input-group-prepend">
          <span class="input-group-text ${css.spanTitleText}">
            ${contractName} at ${addressLink ? yo`<a href="${addressLink}" target="_blank" rel="noopener noreferrer" onclick=${(e) => e.stopPropagation()}>${shortAddress}</a>` : shortAddress} (${context})
          </span>
        </div>
        <div class="btn-group">
          <button class="btn p-1 btn-secondary">${copyToClipboard(() => base58Address)}</button>
        </div>
      </div>
    </div>
  `
  ...
  var close = yo`
    <button
      class="${css.udappClose} mr-1 p-1 btn btn-secondary align-items-center tooltip-above ta-right ta-clear"
      data-id="universalDappUiUdappClose"
      onclick=${remove}
      data-title="Permanently remove from the list"
    >
  ...
  function sendData () {
    ...
    // tronweb trick
    const funcHash = `0x${calldata.slice(0, 8)}`
