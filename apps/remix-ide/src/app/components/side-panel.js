...
export class SidePanel extends AbstractPanel {
  ...
  addView (profile, view, options = {}) {
    this.localProfiles[profile.name] = profile
    super.addView(profile, view)
    this.verticalIcons.linkContent(profile, options)
  }
  ...
  async renderHeader () {
    ...
    if (this.active) {
      let profile
      try {
        if (typeof window !== 'undefined' && /[?&]mockGetProfileFail=1/.test(window.location.search)) {
          throw new Error('mockGetProfileFail (e2e fixture)')
        }
        profile = await this.appManager.getProfile(this.active)
      } catch (e) {
        profile = this.localProfiles[this.active]
      }
      if (!profile) return
