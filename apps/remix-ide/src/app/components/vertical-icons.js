...
export class VerticalIcons extends Plugin {
  ...
  linkContent (profile, options = {}) {
    if (!profile.icon) return
    this.addIcon(profile)
    if (!options.skipStatusListener) this.listenOnStatus(profile)
  }
  ...
  listenOnStatus (profile) {
    ...
    const keys = ['edited', 'succeed', 'none', 'loading', 'failed', '200+']
    ...
    const fn = (status) => {
      ...
      if (typeof status.key === 'string' && (!keys.includes(status.key)) && !/^\d\+?$/.test(status.key)) {
        throw new Error('key should contain either number, result count, or ' + keys.join())
      }
  ...
  setIconStatus (name, status) {
    ...
    el.querySelectorAll('.sr-only').forEach((srOnly) => srOnly.remove())
    ...
    if (typeof status.key === 'number') {
      ...
      key = helper.checkSpecialChars(status.key) ? '' : status.key
      // Reserved keys map to FontAwesome icons via ::before pseudo-element
      // (see resolveClasses). Writing the key as text into the <i> element
      // would render literal words like "succeed" / "edited" next to the
      // icon -- visible as garbled text when clipped by the icon container.
      // Keep text empty for those reserved keys; only string keys that
      // fall through to the default branch (e.g. badge labels) keep the
      // text content.
      const RESERVED_ICON_KEYS = ['succeed', 'edited', 'loading', 'failed']
      text = RESERVED_ICON_KEYS.includes(key) ? '' : key
    }
  ...
  toggle (name) {
    ...
    if (name === 'globalSearch') gtag('event', 'search_entry_clicked', { from: 'activity_bar' })
    gtag('event', 'click', { event_category: 'menu_user_action', event_label: `toggle_content_${name}` })
  }
  ...
  async itemContextMenu (e, name, documentation) {
    const actions = {}
    const isActive = await this.appManager.isActive(name)
    if (isActive && await this.appManager.canDeactivatePlugin(profile, { name })) {
      actions.Deactivate = () => {
  ...
  aiAssistantClick () {
    this.call('aiPanel', 'hide')
  }

  render () {
    const aiAssistant = yo`<div id='aiAssistantIcons' onclick="${this.aiAssistantClick.bind(this)}" class="m-2 ${css.iconAi}" title="AI Assistant" data-id="verticalIconsAiAssistantIcons" id="verticalIconsAiAssistantIcons">
