...
module.exports = {
  ...
  'Should launch debugger': function (browser: NightwatchBrowser) {
    browser.addFile('blah.sol', sources[0]['blah.sol'])
      ...
      .verifyContracts(['Kickstarter'])
      .clickLaunchIcon('udapp')
      .selectContract('Kickstarter')
  ...
  'Should debug transaction using slider': function (browser: NightwatchBrowser) {
    browser.waitForElementVisible('*[data-id="verticalIconsKindudapp"]')
      ...
      .waitForElementContainsText('*[data-id="stepdetail"]', 'vm trace step:', 60000)
  },
  ...
  'Should step back and forward transaction': function (browser: NightwatchBrowser) {
    browser.waitForElementVisible('*[data-id="verticalIconsKindudapp"]')
      ...
      .pause(2000)
      .waitForElementContainsText('*[data-id="stepdetail"]', 'vm trace step:', 60000)
      .waitForElementContainsText('*[data-id="stepdetail"]', 'execution step:', 60000)
  ...
  'Should display correct source highlighting while debugging a contract which has ABIEncoderV2': function (browser: NightwatchBrowser) {
    ...
    */
      .waitForElementPresent('[class*="highlightLine"]')
      .goToVMTraceStep(266)
      .pause(1000)
  ...
  'Should call the debugger api: getTrace': function (browser: NightwatchBrowser) {
    browser.getLastTransactionHash((hash) => {
      browser
        .addFile('test_jsGetTrace.js', { content: jsGetTrace(hash) })
        .executeTerminalScript('remix.exeCurrent()')
        .waitForElementContainsText('*[data-id="terminalJournal"]', 'structLogs', 60000)
    })
  },
  ...
  'Should call the debugger api: debug': function (browser: NightwatchBrowser) {
    browser.getLastTransactionHash((hash) => {
      browser
        .addFile('test_jsDebug.js', { content: jsDebug(hash) })
        .executeTerminalScript('remix.exeCurrent()')
        .pause(3000)
...
function removeWebpackDevOverlay () {
...
const jsGetTrace = (hash: string) => `(async () => {
  try {
      const result = await remix.call('debugger', 'getTrace', '${hash}')
...
const jsDebug = (hash: string) => `(async () => {    
  try {
      const result = await remix.call('debugger', 'debug', '${hash}')
      console.log('result ', result)
  } catch (e) {
      console.log(e.message)
  }
